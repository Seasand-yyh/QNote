...
* Spring核心主要分为两部分。
  ...
  * IOC：控制反转。比如有一个类，在类里面有方法（不是静态的方法），要调用里面的方法，传统的做法是创建类的对象，使用对象调用方法。创建类对象的过程，需要使用new关键字。在IOC中对象的创建不是通过new方式实现，而是交给Spring来创建类的对象。
...
<beans xmlns="http://www.springframework.org/schema/beans" 
	...
	<bean id="user" class="cn.seasand.code.UserFactory" factory-method="getUser"></bean>
</beans>
~~~
...
<beans xmlns="http://www.springframework.org/schema/beans" 
	...
	<bean id="userFactory" class="cn.seasand.code.UserFactory"></bean>
...
<beans xmlns="http://www.springframework.org/schema/beans" 
	...
	<bean id="user" class="cn.seasand.code.User">
...
<beans xmlns="http://www.springframework.org/schema/beans" 
	...
	<bean id="book" class="cn.seasand.code.Book">
...
![1551059308594](images/1551059308594.png)
...
<beans xmlns="http://www.springframework.org/schema/beans" 
	...
	<bean id="book" class="cn.seasand.code.Book" p:bookname="Thinking in Java"></bean>
    <bean id="user" class="cn.seasand.code.User" p:book-ref="book"></bean>
</beans>
~~~
...
* 在Spring里面进行AOP操作，使用AspectJ实现。AspectJ是一个面向切面的框架，它扩展了Java语言。AspectJ定义了AOP语法所以它有一个专门的编译器用来生成遵守Java字节编码规范的Class文件。AspectJ是一个基于Java语言的AOP框架。AspectJ不是Spring的一部分，和Spring一起使用进行AOP操作。Spring2.0以后新增了对AspectJ的支持。

* AOP操作相关jar包

![1551081133674](images/1551081133674.png)
...
![1551160395057](images/1551160395057.png)
...
![1551160533086](images/1551160533086.png)
...
Spring事务管理API：针对不同的DAO层框架，提供接口不同的实现。
...
* 导入相关jar包
  ...
  ![1551086406318](images/1551086406318.png)
